...
2. `TraceCurveObj3D`
    ...
    - `LayerObj3D` での細くなりすぎる問題を克服
        - ![TraceCurveObj3D と LayerObj3D を比較した画像. LayerObj3D が細くなっているのに対し, TraceCurveObj3D は均等になっている.](./img/layer_vs_center.png)
    - 正確な断面の再現は不向き
    - 課題
        - 急激なカーブでの自己交差
            - 小規模の自己交差については中心点を少しずらす
            - ![急カーブで自己交差が発生している. 自己交差が起きている面を赤く表示している.](img/intersect00.png)
            - ![赤と青の二二重振りのモデルがある. 赤が自己交差があるもので青が中心点をずらして解決したもの.](img/intersect_diff.png)
            - ワイヤーの半径を手動で調整
    - 適している対象の例
        - 二重振り子
3. `MultiObj3D`
    ...
    - `TraceCurveObj3D` は土台の方向に端を曲げることでプリントした時の強度を増やす
        - ![二つのオブジェクトが端を曲げられた状態で土台に接続している](img/stand_connect00.png)
